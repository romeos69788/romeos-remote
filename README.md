...
**Εκτός σπιτιού:** το `npm run dev` τρέχει μόνο τοπικά· για μόνιμο URL από internet χρειάζεται hosting (`npm run build` + Netlify/Vercel κ.λπ.) ή tunnel (ngrok). Το **MQTT** (HiveMQ `wss://…`) δουλεύει από το κινητό όπου κι αν είσαι, αρκεί η σελίδα να φορτώνει και τα credentials να είναι σωστά.
